package xuanmo.arcartxsuite.onlinerewards.redis;
...
import xuanmo.arcartxsuite.onlinerewards.config.OnlineRewardsRedisConfiguration;

public final class OnlineRewardsRedisBridge {

    private final JavaPlugin plugin;
    private final OnlineRewardsRedisConfiguration configuration;
    ...
    public OnlineRewardsRedisBridge(JavaPlugin plugin, OnlineRewardsRedisConfiguration configuration, Consumer<String> messageConsumer) {
        this.plugin = plugin;
    ...
    public boolean start() {
        ...
        try {
            ...
            subscriberExecutor = Executors.newSingleThreadExecutor(runnable -> {
                Thread thread = new Thread(runnable, "ArcartXSuite-OnlineRewards-Redis");
            ...
            plugin.getLogger().warning("初始化 OnlineRewards Redis 广播失败: " + exception.getMessage());
            shutdown();
            return false;
        }
    }
    ...
    public void publish(String payload) {
        ...
        try (Jedis jedis = pool.getResource()) {
            ...
            plugin.getLogger().warning("OnlineRewards Redis 发布失败: " + exception.getMessage());
        }
    }
    ...
    private void subscribeLoop() {
        while (active && pool != null && pubSub != null) {
            try (Jedis jedis = pool.getResource()) {
                ...
                if (active) {
                    plugin.getLogger().warning("OnlineRewards Redis 订阅中断，3 秒后重试: " + exception.getMessage());
                    try {
                        Thread.sleep(3000L);
