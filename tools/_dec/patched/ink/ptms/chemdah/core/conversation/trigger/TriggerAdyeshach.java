...
public final class TriggerAdyeshach {
    ...
    public final void onAdyInteract(@NotNull AdyeshachEntityInteractEvent e) {
        ...
        DebugHandlerKt.debug((Entity)player2, Debug.CONVERSATION, "Adyeshach \u4ea4\u4e92\u4e8b\u4ef6\uff0c\u5b9e\u4f53: " + e.getEntity().getId() + ", \u4e3b\u624b: " + e.isMainHand());
        if (e.isMainHand() && ChemdahAPI.INSTANCE.getConversationSession(player2) == null) {
            String[] stringArray = new String[]{e.getEntity().getId()};
            Conversation conversation2 = ConversationManager.INSTANCE.getConversation(player2, "adyeshach", e.getEntity(), stringArray);
            if (conversation2 != null) {
                e.setCancelled(true);
                stringArray = new String[]{"NO_MOVE"};
                if (conversation2.hasFlag(stringArray) && !player2.isOnGround()) {
                    DebugHandlerKt.debug((Entity)player2, Debug.CONVERSATION, "\u73a9\u5bb6\u5728\u7a7a\u4e2d\uff0c\u5ffd\u7565 NO_MOVE \u5bf9\u8bdd");
                    return;
                }
                DebugHandlerKt.debug((Entity)player2, Debug.CONVERSATION, "\u540c\u6b65\u6253\u5f00 Adyeshach \u5bf9\u8bdd: " + conversation2.getId());
                TriggerAdyeshachKt.openByAdyeshach(conversation2, player2, e.getEntity(), false);
            } else {
                DebugHandlerKt.debug((Entity)player2, Debug.CONVERSATION, "\u672a\u627e\u5230\u5339\u914d\u7684\u5bf9\u8bdd");
            }
        } else if (!e.isMainHand()) {
            DebugHandlerKt.debug((Entity)player2, Debug.CONVERSATION, "\u526f\u624b\u4ea4\u4e92\uff0c\u5ffd\u7565");
        } else if (ChemdahAPI.INSTANCE.getConversationSession(player2) != null) {
            DebugHandlerKt.debug((Entity)player2, Debug.CONVERSATION, "\u73a9\u5bb6\u5df2\u6709\u6d3b\u52a8\u4f1a\u8bdd\uff0c\u963b\u6b62\u65b0\u5bf9\u8bdd");
            Session session = ChemdahAPI.INSTANCE.getConversationSession(player2);
            Intrinsics.checkNotNull((Object)session);
            Session session2 = session;
            DebugHandlerKt.debug((Entity)player2, Debug.CONVERSATION, "\u4f1a\u8bdd: " + session2.getConversation().getId() + ", \u6709\u6548: " + session2.isValid() + ", \u5df2\u5173\u95ed: " + session2.isClosed());
        }
    }
}
