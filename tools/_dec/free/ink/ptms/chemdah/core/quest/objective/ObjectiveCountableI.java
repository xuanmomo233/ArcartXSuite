...
extends Objective<E> {
    public ObjectiveCountableI() {
        this.addGoal("amount", "Number", ObjectiveCountableI::_init_$lambda$0);
    ...
    private static final Boolean _init_$lambda$0(PlayerProfile profile, Task task) {
        ...
        Data $this$cint$iv = task.getGoal().get("amount");
        boolean $i$f$getCint = false;
        return profile.dataOperator(task).more("amount", RangesKt.coerceAtLeast((int)Coerce.toInteger((Object)$this$cint$iv), (int)1));
    }
