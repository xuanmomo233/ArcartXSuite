 ...
 *  ink.ptms.chemdah.taboolib.library.xseries.XPotion
...
import ink.ptms.chemdah.taboolib.library.xseries.XPotion;
...
public final class ConversationListener {
    ...
    private final void onTick() {
        ...
        while (iterator.hasNext()) {
            ...
            if (session.isClosed()) {
                ConversationManager.INSTANCE.getSessions().remove(name);
                Object[] objectArray = new Object[]{"\u6e05\u7406\u50f5\u5c38\u4f1a\u8bdd: " + name};
                IOKt.info((Object[])objectArray);
                continue;
            }
            Conversation conversation2 = session.getConversation();
            if (!(conversation2.getTheme() instanceof WorldSide)) continue;
            ...
            if (!anotherWorld && !turnHead) {
                if (!tooFar) continue;
            }
            if (anotherWorld) {
                DebugHandlerKt.debug((Entity)session.getPlayer(), Debug.CONVERSATION, "\u73a9\u5bb6\u5207\u6362\u4e16\u754c\uff0c\u5173\u95ed\u4f1a\u8bdd");
            } else if (turnHead) {
                DebugHandlerKt.debug((Entity)session.getPlayer(), Debug.CONVERSATION, "\u73a9\u5bb6\u626d\u5934\uff0c\u5173\u95ed\u4f1a\u8bdd");
            } else if (tooFar) {
                DebugHandlerKt.debug((Entity)session.getPlayer(), Debug.CONVERSATION, "\u73a9\u5bb6\u8ddd\u79bb\u8fc7\u8fdc (" + session.getDistance() + " > " + closeDistance + ")\uff0c\u5173\u95ed\u4f1a\u8bdd");
            }
    ...
    private final void onBegin(ConversationEvents.Begin e) {
        if (e.getConversation().getTheme() instanceof WorldSide) {
            ...
            if (e.getConversation().hasFlag((String[])$this$forEach$iv) && !playerSpeed.containsKey(e.getSession().getPlayer().getName())) {
    ...
    private final void onDamage(EntityDamageEvent e) {
        if (e.getEntity() instanceof Player) {
            ...
            if (session2.getConversation().hasFlag(stringArray)) {
                DebugHandlerKt.debug((Entity)player2, Debug.CONVERSATION, "\u5bf9\u8bdd\u6709 FORCE_DISPLAY \u6807\u8bb0\uff0c\u53d6\u6d88\u4f24\u5bb3");
                e.setCancelled(true);
                return;
            }
            DebugHandlerKt.debug((Entity)player2, Debug.CONVERSATION, "\u73a9\u5bb6\u53d7\u4f24\uff0c\u5173\u95ed\u4f1a\u8bdd");
            session2.close(true);
        }
    }
    ...
    private final void onDropItem(PlayerDropItemEvent e) {
        ...
        if (session2.getConversation().hasFlag(stringArray)) {
            ...
            DebugHandlerKt.debug((Entity)player3, Debug.CONVERSATION, "\u5bf9\u8bdd\u6709 FORCE_DISPLAY \u6807\u8bb0\uff0c\u963b\u6b62\u4e22\u5f03\u7269\u54c1");
            e.setCancelled(true);
        }
    }
        ...
        if (StringsKt.startsWith$default((String)string, (String)"/session", (boolean)false, (int)2, null)) {
            ...
            if (args.size() == 2 && Intrinsics.areEqual(args.get(0), (Object)"reply")) {
                ...
                block5: {
                    Player player2 = e.getPlayer();
                    Intrinsics.checkNotNullExpressionValue((Object)player2, (String)"e.player");
                    session = ChemdahAPI.INSTANCE.getConversationSession(player2);
                    if (session == null) {
                        Player player3 = e.getPlayer();
                        Intrinsics.checkNotNullExpressionValue((Object)player3, (String)"e.player");
                        DebugHandlerKt.debug((Entity)player3, Debug.CONVERSATION, "/session reply \u5931\u8d25: \u65e0\u6d3b\u52a8\u4f1a\u8bdd");
                        return;
                    }
                ...
                PlayerReply reply = v4;
                if (reply == null) {
                    Player player4 = e.getPlayer();
                    Intrinsics.checkNotNullExpressionValue((Object)player4, (String)"e.player");
                    DebugHandlerKt.debug((Entity)player4, Debug.CONVERSATION, "/session reply \u5931\u8d25: \u672a\u627e\u5230\u56de\u590d ID " + (String)args.get(1));
                    return;
                }
                Player player5 = e.getPlayer();
                Intrinsics.checkNotNullExpressionValue((Object)player5, (String)"e.player");
                DebugHandlerKt.debug((Entity)player5, Debug.CONVERSATION, "/session reply \u89e6\u53d1\u56de\u590d\u9009\u62e9");
                FuturesKt.applyWithError(reply.check(session), (Function1)new Function1<Boolean, Unit>(reply, session, e){
                    final /* synthetic */ PlayerReply $reply;
                    final /* synthetic */ Session $session;
                    final /* synthetic */ PlayerCommandPreprocessEvent $e;
                    {
                        this.$reply = $reply;
                        this.$session = $session;
                        this.$e = $e;
                    ...
                    public final void invoke(boolean cond) {
                        if (cond) {
                            ...
                            Intrinsics.checkNotNullExpressionValue((Object)player2, (String)"e.player");
                            DebugHandlerKt.debug((Entity)player2, Debug.CONVERSATION, "/session reply \u5931\u8d25: \u6761\u4ef6\u4e0d\u6ee1\u8db3");
                        }
                    }
                });
            }
    ...
    private final void onPlayerCastSkillEvent(OptionalEvent event) {
        ...
        if (ChemdahAPI.INSTANCE.getConversationSession(player2) != null) {
            Player player3 = e.getPlayer();
            Intrinsics.checkNotNullExpressionValue((Object)player3, (String)"e.player");
            DebugHandlerKt.debug((Entity)player3, Debug.CONVERSATION, "\u5bf9\u8bdd\u4e2d\u963b\u6b62 SkillAPI \u6280\u80fd\u91ca\u653e");
            e.setCancelled(true);
        }
    }

    private final void sendFakeFoodLevel(Player player2, int foodLevel) {
        block2: {
            try {
                MinecraftServerUtilKt.sendPacket((Player)player2, (Object)new PacketPlayOutUpdateHealth((float)player2.getHealth(), foodLevel, 0.0f));
            }
            catch (Throwable ex) {
    ...
    static {
        ...
        pairArray[0] = TuplesKt.to((Object)PotionEffectType.BLINDNESS, (Object)0);
        PotionEffectType potionEffectType = XPotion.SLOWNESS.get();
        Intrinsics.checkNotNull((Object)potionEffectType);
        pairArray[1] = TuplesKt.to((Object)potionEffectType, (Object)2);
